...
export class EthTester {
  ...
  genSignedTransaction = async (options = DEFAULT_TRANSACTION, txType) => {
    ...
    this.logger.debug(
      `Tx [${/:([0-9]+)$/.exec(this.web3.currentProvider.host)?.[1]}] from: ${`${data.from.substr(0, 5)}...${data.from.substr(data.from.length - 3)}`}, ${data.to ? `to: ${`${data.to.substr(0, 5)}...${data.to.substr(data.to.length - 3)}`}, ` : ""}${data.value ? `value: ${data.value.toString()}, ` : ""}${data.gasPrice ? `gasPrice: ${data.gasPrice.toString()}, ` : ""}${data.maxFeePerGas ? `maxFeePerGas: ${data.maxFeePerGas.toString()}, ` : ""}${
        data.maxPriorityFeePerGas
          ? `maxPriorityFeePerGas: ${data.maxPriorityFeePerGas.toString()}, `
          : ""
      }${data.accessList ? `accessList: ${data.accessList.toString()}, ` : ""}${data.gas ? `gas: ${data.gas.toString()}, ` : ""}${data.nonce ? `nonce: ${data.nonce.toString()}, ` : ""}${
        !data.data
          ? ""
          : `data: ${
              data.data.length < 50
