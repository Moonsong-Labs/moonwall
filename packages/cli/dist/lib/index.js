...
async function getGithubReleaseBinary(url, binaryPath) {
  if (!fs11.existsSync(binaryPath)) {
    ...
    child_process2.execSync(
      `mkdir -p ${path10.dirname(binaryPath)} && wget -q ${url} -O ${binaryPath} && chmod u+x ${binaryPath}`
    );
...
async function getMoonbeamReleaseBinary(binaryTag) {
  const binaryPath = path10.join(BINARY_DIRECTORY, `moonbeam-${binaryTag}`);
...
async function getPolkadotReleaseBinary(binaryTag) {
  const binaryPath = path10.join(BINARY_DIRECTORY, `polkadot-${binaryTag}`);
  return getGithubReleaseBinary(
...
async function getTagSha8(binaryTag) {
  const sha = child_process2.execSync(`git rev-list -1 ${binaryTag}`).toString();
  if (!sha) {
...
async function getMoonbeamDockerBinary(binaryTag) {
  ...
  const binaryPath = path10.join(BINARY_DIRECTORY, `moonbeam-${sha8}`);
  if (!fs11.existsSync(binaryPath)) {
    ...
    console.log(`     Missing ${binaryPath} locally, downloading it...`);
    child_process2.execSync(
      `mkdir -p ${path10.dirname(binaryPath)} &&           docker create --pull always --name moonbeam-tmp ${dockerImage} &&           docker cp moonbeam-tmp:/moonbeam/moonbeam ${binaryPath} &&           docker rm moonbeam-tmp`
    );
    console.log(`${binaryPath} downloaded !`);
  }
  return binaryPath;
...
async function generateSpecs(binaryPath, runtimeName, raw) {
  const specPath = path10.join(
    SPECS_DIRECTORY,
    `${runtimeName}-${raw ? "raw" : "plain"}-specs.json`
  );
  child_process2.execSync(
    `mkdir -p ${path10.dirname(specPath)} && ${binaryPath} build-spec --chain ${runtimeName} ${raw ? "--raw" : ""} --disable-default-bootnode > ${specPath}`
  );
  return specPath;
}
