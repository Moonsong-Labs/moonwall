...
var LaunchCommandParser = class _LaunchCommandParser {
  ...
  constructor(options) {
    ...
    this.args = launchSpec.options
      ? [...launchSpec.options]
      : fetchDefaultArgs(path3.basename(launchSpec.binPath), additionalRepos);
  }
  overrideArg(newArg) {
...
var MoonwallContext = class _MoonwallContext {
  ...
  async startZombieNetwork() {
    ...
    const ipcLogPath = path10.join(network.tmpDir, "ipc-server.log");
...
import { alith as alith3 } from "@moonwall/util";
