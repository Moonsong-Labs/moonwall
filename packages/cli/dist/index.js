...
async function getMoonbeamDockerBinary(binaryTag) {
  ...
  if (!fs.existsSync(binaryPath)) {
    ...
    console.log(`     Missing ${binaryPath} locally, downloading it...`);
    child_process.execSync(
      `mkdir -p ${path.dirname(binaryPath)} &&           docker create --pull always --name moonbeam-tmp ${dockerImage} &&           docker cp moonbeam-tmp:/moonbeam/moonbeam ${binaryPath} &&           docker rm moonbeam-tmp`
    );
    console.log(`${binaryPath} downloaded !`);
  }
  return binaryPath;
  ...
  if (binArch !== currentArch && binArch !== "unknown") {
    throw new Error(
      `The binary architecture ${chalk3.bgWhiteBright.blackBright(
        binArch
      )} does not match this system's architecture ${chalk3.bgWhiteBright.blackBright(currentArch)}
Download or compile a new binary executable for ${chalk3.bgWhiteBright.blackBright(currentArch)} `
    );
  }
  return true;
}
...
var execAsync2 = promisify(child_process2.exec);
...
async function getWsFromConfig(providerName) {
  if (providerName) {
    ...
    if (typeof provider2 === "undefined") {
      throw new Error(`Cannot find provider ${chalk7.bgWhiteBright.blackBright(providerName)}`);
    }
    if (!provider2.ws) {
  ...
  if (typeof provider === "undefined") {
    throw new Error(
      `Cannot find providers of type ${chalk7.bgWhiteBright.blackBright("polkadotJs")}`
    );
  }
  if (!provider.ws) {
    throw new Error("Provider does not have an attached ws() property ");
...
var MoonwallContext = class _MoonwallContext {
  ...
  async startZombieNetwork() {
    ...
    const ipcLogPath = path11.join(network.tmpDir, "ipc-server.log");
    const ipcLogger = fs12.createWriteStream(ipcLogPath, { flags: "a" });
  ...
  async handleZombiePostConnection(silent, env) {
    ...
    if (!isOptionSet("disableLogEavesdropping")) {
      ...
      readStreams = zombieNodeLogs.map((logPath) => {
        const readStream = fs12.createReadStream(logPath, { encoding: "utf8" });
...
`;
          try {
            fs12.appendFileSync(logLocation, message);
          } catch (err) {
...
`;
          try {
            fs12.appendFileSync(proc.logPath, message);
          } catch (err) {
...
var notePreimage = async (context, proposal, account = alith2) => {
...
var whiteListTrackNoSend = async (context, proposal) => {
  ...
  let proposalId;
  filterAndApply2(result.events, "referenda", ["Submitted"], (found) => {
...
var execOpenTechCommitteeProposal = async (
  ...
  filterAndApply2(result2.events, "openTechCommitteeCollective", ["Proposed"], (found) => {
    ...
    await context
      ...
      .tx.convictionVoting.vote(refIndex, {
        Standard: {
          ...
          balance: (await context.polkadotJs().query.system.account(alith2.address)).data.free,
        },
      })
      .paymentInfo(alith2)
  ).partialFee;
...
import fs13, { existsSync as existsSync2, readFileSync as readFileSync3 } from "fs";
async function upgradeRuntimeChopsticks(context, path12, providerName) {
  ...
  const rtWasm = readFileSync3(path12);
...
`);
      ...
      log(`Waiting to apply new runtime (${chalk10.red("~4min")})...`);
...
var chopsticksHandler = ({ testCases, context, testCase, logger: logger6 }) => {
  ...
  const newKeyring = () => {
    ...
    return {
      alice: keyring.addFromUri(isEth ? ALITH_PRIVATE_KEY3 : "//Alice", {
        name: "Alice default",
      }),
