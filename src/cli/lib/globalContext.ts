...
import {
  ...
  parseZombieCmd,
} from "../internal/commandParsers.js";
import {
  checkZombieBins,
  getZombieConfig,
  ...
  public async startNetwork() {
        ...
        Promise.all(
          env.foundation.launchSpec.map(async (spec, index) => {
            try {
              ...
              throw new Error(`Failed to start chopsticks '${spec.configPath}': ${error.message}`, { cause: error });
            }
          })
        ),
        maxStartupTimeout
    ...
    const maxStartupTimeout =
      launchSpec && "useDocker" in launchSpec && launchSpec.useDocker
        ...
        : isForkingNode
          ? 120000 // 2 min for forking (downloads remote state)
          : 30000; // 30s for regular dev nodes
        ...
        nodes.map(async ({ cmd, args, name, launch }) => {
          if (launch) {
            try {
              ...
              throw new Error(`Failed to start node '${name || "unnamed"}': ${error.message}`, { cause: error });
            }
          }
        })
      ),
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt === maxRetries) {
            throw new Error(
              `Failed to connect provider '${provider.name}' after ${maxRetries} attempts: ${error.message}`, { cause: error }
            );
          }
