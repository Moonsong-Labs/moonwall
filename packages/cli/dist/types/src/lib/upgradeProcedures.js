  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      switch (options.upgradeMethod) {
        case "Sudo": {
          log(
            `Sending sudo.setCode (${sha256(Buffer.from(code))} [~${Math.floor(code.length / 1024)} kb])...`
          );
        ...
        case "Governance": {
          ...
          if (api.query.preimage && preImageExists.isSome && preImageExists.unwrap().isRequested) {
            ...
            log(
              `Registering preimage (${sha256(Buffer.from(code))} [~${Math.floor(code.length / 1024)} kb])...`
            );
