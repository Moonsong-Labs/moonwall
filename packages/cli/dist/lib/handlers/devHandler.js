...
import fs11, { readFileSync as readFileSync2 } from "fs";
...
Please specify path to Foundry directory at:  ${chalk9.bgWhiteBright.blackBright(
...
Please ${chalk9.bgWhiteBright.blackBright("recompile contract")} ${contractName}.sol`
    );
  }
...
async function deployCreateCompiledContract(context, contractName, options) {
  ...
  const { privateKey = ALITH_PRIVATE_KEY2, args = [], ...rest } = options || {};
  const blob = {
    ...rest,
    privateKey,
    args,
...
var devHandler = ({ testCases, context, testCase, logger: logger6 }) => {
  ...
  const ctx = {
    ...
    createBlock: async (transactions, options) => {
      const defaults = {
        signer: env.defaultSigner || alith2,
